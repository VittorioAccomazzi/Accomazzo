...
const srcCsv="Data/GENEALOGIA - Birth.csv"
const srcImg= null; // do not copy the image
const dstFld= null; // do not check the image path

// process the record of the 1800. There are two types of records : the one from teh Curch, in italian
// and the ones from the municipality in franch (Piemont was under the Napoleon's empire at the time) 
...
( async  () => {
    ...
    if( dstFld ){
        ...
        await fsPromise.mkdir(dstFld+docDefs.Birth1800.folder, {recursive:true});
    }
        ...
        if( line ){
            try
            {
                ...
                if( items.length > 11 ){
                    ...
                    if( imgPath === year ){
                        // hack -- in the Napoleonic document the year is reported prior
                        // to the image filename
                        imgPath=items[1];
                    }
                    imgPath += ".jpg"
                    let doc ={
                        Url  : docDefs.Birth1800.folder+imgPath,
                        Year : parseInt (year),
                        Name : items[3],
                        Father : items[6],
                        Mother : items[8],
                    ...
                    if( dstFld ){
                        let validImage =  await Utilities.ValidateImage(imgPath, dstFld, docDefs.Birth1800.folder, srcImg);
